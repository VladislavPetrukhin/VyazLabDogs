...
function updateFirstValues() {
  ...
  if (table && attribute) {
    fetch("/get_attribute_values?table=" + table + "&attribute=" + attribute)
      .then((response) => response.json())
      .then((data) => {
      ...
      .catch((error) => console.error("Ошибка при загрузке значений:", error));
  }
}
...
function updateSecondAttributes() {
  ...
  if (table1) {
    fetch("/get_attributes?table=" + table1)
      ...
      .catch((error) =>
        console.error("Ошибка при загрузке атрибутов второй таблицы:", error),
      );
  }
}
...
function updateSecondValues() {
  ...
  if (table0 && firstAttr && firstValue && secondAttr) {
    const url = table1
    ...
    fetch(url)
      ...
      .catch((error) =>
        console.error("Ошибка при загрузке отфильтрованных значений:", error),
      );
  }
}
...
function addValue() {
  const table = document.getElementById("table").value;
  const attribute = document.getElementById(
    currentLevel === "first" ? "first_attribute" : "second_attribute",
  ).value;
  ...
  fetch("/add_value", {
    ...
    .catch((error) => console.error("Ошибка при добавлении значения:", error));
}
