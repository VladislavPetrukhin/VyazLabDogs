...
def add():
    ...
    if request.method == 'POST':
        data = {  # Убрано лишнее вложенное условие
        ...
        with get_db_connection() as conn:  # Добавлен контекст для подключения
            cursor = conn.cursor()
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (data['name'], data['breed_id'], data['location_id'], data['birth_date'], data['registration_date'], data['microchip_number'], data['coat_type'], data['color_variations'], data['temperament'], data['size'], data['getting_id'], data['vet_examinations_id']))
            conn.commit()
            ...
            breeds = cursor.execute("SELECT * FROM breeds").fetchall()  # Упрощено
            locations = cursor.execute("SELECT * FROM locations").fetchall()  # Упрощено
    else:
        with get_db_connection() as conn:
            dogs = conn.execute("""
                SELECT d.id, d.name, b.breed_name, l.location_name, d.birth_date, d.registration_date, d.microchip_number
